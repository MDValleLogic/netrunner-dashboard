...
export default function RouteRunnerConfig() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(`/api/routerunner/config?device_id=${selectedDeviceId}`)
      .then(r => r.json())
  ...
  async function save() {
    ...
    try {
      const res = await fetch("/api/routerunner/config", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ device_id: selectedDeviceId, targets, interval_seconds: interval }) });
      const j = await res.json();
      if (!j.ok) throw new Error(j.error || "Save failed");
      setMsg({ type: "success", text: "Config saved — appliance picks up changes on next cycle" });
  ...
  const selStyle = { background: "#111827", border: "1px solid #374151", borderRadius: 6, color: "#e5e7eb", padding: "8px 10px", fontSize: 12, fontFamily: "monospace" };

  return (
    <div className="min-h-screen bg-gray-950 text-gray-100 p-6">
      <div className="flex items-center justify-between mb-6 max-w-3xl">
        <div className="flex items-center gap-3">
          <div className="p-2 rounded-lg bg-blue-500/10 border border-blue-500/20"><Settings size={20} className="text-blue-400" /></div>
          <div>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          <select value={selectedDeviceId || ""} onChange={e => setSelectedDeviceId(e.target.value)} style={selStyle}>
            {devices.map(d => <option key={d.device_id} value={d.device_id}>{d.nickname ? `${d.nickname} (${d.nr_serial})` : d.nr_serial}</option>)}
          </select>
          <button onClick={save} disabled={saving || !selectedDeviceId} style={{ background: "#1d4ed8", border: "none", borderRadius: 6, color: "#fff", padding: "9px 20px", fontSize: 13, fontWeight: 600, cursor: saving ? "not-allowed" : "pointer", opacity: saving ? 0.6 : 1, fontFamily: "monospace" }}>{saving ? "Saving…" : "Save Config"}</button>
        </div>
      </div>

      <div className="max-w-3xl space-y-4">
        {msg && <div style={{ padding: "10px 16px", borderRadius: 8, fontSize: 13, background: msg.type === "success" ? "rgba(34,197,94,0.1)" : "rgba(239,68,68,0.1)", border: `1px solid ${msg.type === "success" ? "rgba(34,197,94,0.3)" : "rgba(239,68,68,0.3)"}`, color: msg.type === "success" ? "#22c55e" : "#ef4444" }}>{msg.type === "success" ? "✓" : "⚠"} {msg.text}</div>}
        ...
        <div className="rounded-lg border border-gray-700/60 bg-gray-900/60 p-4">
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {INTERVAL_OPTIONS.map(opt => (
              <button key={opt.value} onClick={() => setIntervalS(opt.value)} style={{ padding: "8px 18px", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: "pointer", border: interval === opt.value ? "2px solid #3b82f6" : "1px solid #374151", background: interval === opt.value ? "rgba(59,130,246,0.1)" : "#111827", color: interval === opt.value ? "#60a5fa" : "#9ca3af" }}>{opt.label}</button>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-lg border border-gray-700/60 bg-gray-900/60 p-4">
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <input placeholder="Add custom IP or hostname…" value={newTarget} onChange={e => setNewTarget(e.target.value)} onKeyDown={e => e.key === "Enter" && addTarget(newTarget)} style={{ flex: 1, background: "#111827", border: "1px solid #374151", borderRadius: 6, color: "#e5e7eb", padding: "8px 10px", fontSize: 12, fontFamily: "monospace" }} />
          ...
          <div className="rounded-lg border border-gray-700/60 bg-gray-900/60 p-4">
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 8 }}>
              {available.map(preset => (
                <button key={preset.value} onClick={() => addTarget(preset.value)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", borderRadius: 8, cursor: "pointer", background: "#111827", border: "1px solid #1f2937", textAlign: "left" }}>
